...
contract PaymentPool is Ownable, MerkleProof {
	...
	constructor(ERC20 _token) public {
		token = _token;
		currentCycleStartBlock = block.number;
	}

	function startNewPaymentCycle() internal onlyOwner returns(bool) {
	    require(block.number > currentCycleStartBlock);

	    emit PaymentCycleEnded(numPaymentCycles,
	    				  currentCycleStartBlock,
	    				  block.number);
	    numPaymentCycles = numPaymentCycles.add(1);
	    currentCycleStartBlock = block.number.add(1);
	    return true;
	}
	...
	function submitPayeeMerkleRoot(
 	...
 	function balanceForProofWithAddress(
	    ...
	    if (withdrawals[_address] < cumulativeAmount &&
	        verifyProof(_proof, payeeRoots[paymentCycleNumber], leaf))
	    {
