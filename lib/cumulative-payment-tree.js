...
export default class CumulativePaymentTree extends MerkleTree {
  constructor(paymentList) {
    let filteredPaymentList =
        paymentList.filter(payment => payment.payee && payment.amount);
    let groupedPayees =
        _.groupBy(filteredPaymentList, payment => payment.payee);
    let reducedPaymentList = Object.keys(groupedPayees).map(payee => {
      let payments = groupedPayees[payee];
      let amount =
          _.reduce(payments, (sum, payment) => sum + payment.amount, 0);
      return {payee, amount};
    });
    let paymentNodes =
        reducedPaymentList.map(payment => payment.payee + ',' + payment.amount);
  ...
  hexProofForPayee(payee, paymentCycle) {
    let leaf =
        this.paymentNodes.find(paymentNode => paymentNode.indexOf(payee) > -1);
    if (!leaf) {
