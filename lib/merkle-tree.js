import {bufferToHex, setLength, sha3, toBuffer} from 'ethereumjs-util';
...
export default class MerkleTree {
  ...
  getRoot() { return this.layers[this.layers.length - 1][0]; }

  getHexRoot() { return bufferToHex(this.getRoot()); }
  ...
  getProof(el, prefix) {
    ...
    if (prefix) {
      ...
      prefix = prefix.map(item => setLength(toBuffer(item), 32));
  ...
  bufDedup(elements) {
    return elements.filter(
        (el, idx) => { return this.bufIndexOf(el, elements) === idx; });
  }
